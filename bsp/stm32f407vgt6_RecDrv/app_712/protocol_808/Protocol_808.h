...
//--------------  串口透传信息 ---------
typedef struct _RAW_DATATRANS
{
   u8    TYPE;  // 类型
   u8    Enable_SD_Flag; // 使能发送标志位 
   u16   datalen;// 数据信息长度
   u8    RawData[300];
}RAW_DATATRANS;
...
//-------位置字符串 ------
typedef struct _POS_STR
{
   u8  DateTime[20];  //  日期时间字符串  
   u8  Lati_Str[25];  //  纬度字符串
   u8  Longi_Str[25]; //  经度 字符串

}POS_STR;

//---------- POS_STR  ----------------
extern POS_STR   Posit_Str;  //  位置字符串


//---------- RAW_DATATRANS--------
extern RAW_DATATRANS   RawTrans_Com1;
extern RAW_DATATRANS   RawTrans_Com2;   
...
extern u16  Sleep_IO_state; //   休眠情况下的IO 状态   
...
extern void  StatusReg_SPD_PreWARN(void);
extern void  StatusReg_SPD_PreNORMAL(void);
extern void   StatusReg_Tired_PreWARN(void);
extern void   StatusReg_Tired_PreNORMAL(void);
extern void  Recorder_init(void); 
extern u8    Stuff_RecoderACK_0700H_Error(void);
extern void  RAW_TransInit(void); 
