...
//----------  Basic  Config---------------------------
u8          RemoteIP_regDial[4]={114,242,194,226};//{60,28,50,210};  //  add for 北斗认证
u8          RemoteIP_Dnsr[4]={255,255,255,255};  
u8		RemoteIP_main[4]={114,242,194,226};//{125,38,185,88};//{113,31,28,101 };//{113,31,92,200};//天津{60,28,50,210}; 河北天地通 113,31,28,100                        
u16		RemotePort_main=8217; //8499;// 7804;//9131;//天津9131;   河北天地通 8201              //test tianjin    
u8		RemoteIP_aux[4]={202,96,42,113};    //{60,28,50,210}  //IC 卡认证服务中心地址
                                                                         //   202.96.42.113:11001 
u16		   RemotePort_aux=11001;  
u8           APN_String[30]="CMNET"; //"CMNET";   //  河北天地通  移动的卡
u8           DomainNameStr[50]="jr2.gghypt.net"; ;  // 域名  天地通up.gps960.com //jt1.gghypt.net
u8           DomainNameStr_aux[50]="jr2.gghypt.net";     //"www.sina.com";//jt2.gghypt.net 
...
void JT808_DURATION_Init(void)
{
	...
	JT808Conf_struct.DURATION.TCP_ACK_Dur=5;     //  TCP 应答超时
	JT808Conf_struct.DURATION.TCP_ReSD_Num=3;     //  TCP 重发次数
	JT808Conf_struct.DURATION.TCP_ACK_Dur=5;     //  UDP 应答超时
	JT808Conf_struct.DURATION.UDP_ACK_Dur=5;
	JT808Conf_struct.DURATION.UDP_ReSD_Num=3;     //  UDP 重发次数
 	JT808Conf_struct.DURATION.NoDrvLogin_Dur=40;  //  驾驶员没登陆时的发送间隔
 	JT808Conf_struct.DURATION.Sleep_Dur=300;       //  休眠时上报的时间间隔    
...
{
   JT808Conf_struct.DISTANCE.Defalut_DistDelta=500;    // 默认定距回传距离
...
void  JT808_Vehicleinfo_Init(void) 
{
	...
	memcpy(JT808Conf_struct.Vechicle_Info.Vech_Num,"京F34501",8);        
	memcpy(JT808Conf_struct.Vechicle_Info.Vech_Type,"未知型",6);       
	JT808Conf_struct.Vechicle_Info.Dev_ProvinceID=10;  // 默认省ID   0
	JT808Conf_struct.Vechicle_Info.Dev_CityID=1010;      // 默认市ID   0		
...
u8     JT808_Conf_init( void ) 
  {
        ...
        //   2.  Stuff 
		   ...
		   JT808Conf_struct.Msg_Float_ID=0;   // 消息流水号
                ...
                JT808Conf_struct.Speed_warn_MAX=100;           //  速度报警门限
                JT808Conf_struct.Spd_Exd_LimitSeconds=5;  //  超速报警持续时间门限 s
		...
		//--------------------------------------------------------------------------
		  ...
		  // 终端手机号  
		   ...
		   JT808Conf_struct.BD_MaxSpd_preWarnValue=100;    //  超速报警预警差值，单位 1/10  Km/h
		   JT808Conf_struct.BD_TiredDrv_preWarnValue=1200;  // 疲劳驾驶预警差值，单位 秒
...
void TIRED_DoorValue_Init(void)
{
    ...
    TiredConf_struct.TiredDoor.Door_DayAccumlateDrvSec=57600; //8小时
...
void DefaultConfig(void)
{
  ...
  if(JT808Conf_struct.Regsiter_Status)
	  ...
	  //   里程
         ...
         //  速度限制
         rt_kprintf("		   允许最大速度: %d  Km/h  预报警数值 : %d  Km/h  超速报警持续时间门限: %d  s \r\n", JT808Conf_struct.Speed_warn_MAX,JT808Conf_struct.BD_MaxSpd_preWarnValue,JT808Conf_struct.Spd_Exd_LimitSeconds);  		 
