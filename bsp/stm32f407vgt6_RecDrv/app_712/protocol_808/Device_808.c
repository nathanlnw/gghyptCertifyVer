...
void  ACC_status_Check(void)
{
				...
				{	
                    if((Vehicle_RunStatus&0x01)==0x00)
				       rt_kprintf("\r\n  ACC 开\r\n"); 
				...
				{
                   if((Vehicle_RunStatus&0x01)==0x01)
				       rt_kprintf("\r\n  ACC 关 \r\n");
				   Vehicle_RunStatus&=~0x01;
				   //	  ACC OFF	   关火
...
*/
 void TIM2_Configuration(void) //只用一个外部脉冲端口
 {
  ...
  void TIM1_Configuration(void) //只用一个外部脉冲端口
  {
	 ...
	 TIM_TimeBaseStructure.TIM_Period = 0xFFFF;
	 TIM_TimeBaseStructure.TIM_Prescaler=71;   //预分频71，即72分频，得1M	
	 TIM_TimeBaseStructure.TIM_ClockDivision = 0x0;  
	 TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;     
	 ...
	 TIM_OCInitStructure.TIM_Pulse = timer_tim1counter;							//翻转周期：2000个脉冲
