...
//---------- RAW_DATATRANS--------
RAW_DATATRANS   RawTrans_Com1;
RAW_DATATRANS   RawTrans_Com2;   

//---------- POS_STR  ----------------
POS_STR   Posit_Str;  //  位置字符串  
...
u16    Spd_sleep=0;  //  休眠情况下的速度
u32    spd_sleepDur=10;  // 非法位移  
u16    Sleep_IO_state=0; //   休眠情况下的IO 状态
...
{  	
  ...
  u8  packet_type = 0;
           ...
           if((1==GNSS_rawdata.WorkEnable)&&(GNSS_rawdata.save_status))  
           	{
           	      if((GNSS_rawdata.save_status&(1<<GNSS_rawdata.rd_num))&&(GNSS_rawdata.SendFlag==1))   // 检查当前rd   数值状态
           	      	{
    ...
    // ----------------  透传数据--------
    if(RawTrans_Com1.Enable_SD_Flag==1)
    {
        Stuff_DataTrans_0900_BD_Serial1();
        RawTrans_Com1.Enable_SD_Flag=0;
		return true; 
    }
	
    if(RawTrans_Com2.Enable_SD_Flag==1)
    {
        Stuff_DataTrans_0900_BD_Serial2();
        RawTrans_Com2.Enable_SD_Flag=0;
		return true; 
    }
	...
	if((1==Recode_Obj.SD_Data_Flag)&&(1==Recode_Obj.CountStep))
	{
		...
		//           位置数据上报  
			...
			if(stopNormal==2)  
             ...
             if((RdCycle_RdytoSD==ReadCycle_status)&&(0==BD_ISP.ISP_running)&&(DataLink_Status()==1)&&(DEV_Login.Operate_enable==2))    // 读取发送--------- 正常GPS 
...
void Longitude_pro(u8 *tmpinfo)
{
 ...
 GPRMC.longtitude_value=atof((char *)tmpinfo);  
   ...
   memset(Posit_Str.Longi_Str,0,sizeof(Posit_Str.Longi_Str));
   memcpy(Posit_Str.Longi_Str,tmpinfo,10); 
  /*     Latitude  
          dddmm.mmmm
    */
 //--------  808协议  ---------
		 ...
		 // --------  sp 当前是0.1 knot------------------	 
		  ...
		  sp_DISP=sp/100;   //  sp_Disp 单位是 0.1km/h 
	         ...
	         //------------------------------ 通过GPS模块数据获取到的速度 --------------------------------
				  ...
				  if(0==Jia_GPS_spdFlag)	   //  不是加速度时用GPS 速度
       ...
       if(JT808Conf_struct.Speed_GetType)  // 通过速度传感器 获取速度
	   	...
	   	{  // 从GPS 取速度
				 ...
				 if(sp>=5000)		//	过滤零点漂移  速度大于
				 {
                     ...
                     {
                        DistanceAccumulate+=reg;
                        if(DistanceAccumulate>=JT808Conf_struct.DISTANCE.Defalut_DistDelta)
                        	{
                               DistanceAccumulate=0; 
					PositionSD_Enable();          //发送
					Current_UDP_sd=1;
...
void  GPS_Delta_DurPro(void)    //告GPS 触发上报处理函数
{
	...
	{	rt_kprintf("\r\n  %d-%d-%d %d:%d:%d %s",Temp_Gps_Gprs.Date[0],Temp_Gps_Gprs.Date[1],Temp_Gps_Gprs.Date[2],\
	 ...
	 sprintf(Posit_Str.DateTime,"%d-%d-%d %d:%d:%d",Temp_Gps_Gprs.Date[0],Temp_Gps_Gprs.Date[1],Temp_Gps_Gprs.Date[2],\
		Temp_Gps_Gprs.Time[0],Temp_Gps_Gprs.Time[1],Temp_Gps_Gprs.Time[2]);
	 
	// rt_kprintf("\r\n  Datatime: %s |%s|%s|\r\n",Posit_Str.DateTime,Posit_Str.Lati_Str,Posit_Str.Longi_Str); 
	//warn_msg_sd();
...
void StatusReg_SPD_PreWARN(void)
{    //  超速报警
   Warn_Status[2] |= 0x20;//BIT( 13 );
}

void StatusReg_SPD_PreNORMAL(void)
{    //  速度正常
   Warn_Status[2] &=~ 0x20;//BIT( 13 );      00100000 
}

void StatusReg_Tired_PreWARN(void)  
{    //  超速报警
   Warn_Status[2] |= 0x40;//BIT( 14 );
}

void StatusReg_Tired_PreNORMAL(void)
{    //  速度正常
   Warn_Status[2] &=~ 0x40;//BIT( 14 );      01000000 
}
...
//-----------------------------------------------------
	rt_err_t	res;
	   ...
	   if(AppQue.sd_enable_flag==0) 
	   {
	          res = rt_mq_recv( &mq_APPs,(void*)&Rx_reg, 30, RT_TICK_PER_SECOND/50 ); //等待100ms,实际上就是变长的延时,最长100ms
...
{
    if((AppQue.sd_enable_flag==1)&&(AppQue.read_num!=AppQue.write_num))
    	{
             AppQue.abnormal_counter++;
	      if(AppQue.abnormal_counter>=4)  // 2s 
	      	{
	      	    AppQue.abnormal_counter=0;
		    AppQue.re_send_flag=1;
	      	}
    	}
...
void  RAW_TransInit(void)
{
   RawTrans_Com1.datalen=0;
   RawTrans_Com1.Enable_SD_Flag=0;
   RawTrans_Com1.TYPE=0;

   
   RawTrans_Com2.datalen=0;
   RawTrans_Com2.Enable_SD_Flag=0;
   RawTrans_Com2.TYPE=0; 
...
u8  encrypt_lock2_encode( u32 key,  u32  M1,  u32  IA1,  u32 IC1,  unsigned char *buf, u32 len )
{ 
    u32 mkey=0;  
    int i =0 ;
...
u8  Stuff_AccountPacket_0110H(u8  LinkNum,u8 data)  
{
	u8  i=0;
	u16  start_jiami = 0;
	u32 len_jiami = 0;
	//u8  end_jiami;
// 1. Head
  if(!Protocol_Head(MSG_0x0110,Packet_Normal)) 
 	return false; 
 	 //加密标识
 	Original_info[Original_info_Wr++]=data;  
 	 //加密密钥               
    Original_info[Original_info_Wr++] = 0x00;     // 70420                
    Original_info[Original_info_Wr++] = 0x01;
    Original_info[Original_info_Wr++] = 0x13;
    Original_info[Original_info_Wr++] = 0x14;
	//车主联系号码
    start_jiami=Original_info_Wr;  //  加密信息起始地址
    memcpy(Original_info+Original_info_Wr,"15122865297",11);  
    Original_info_Wr+=11;
	Original_info[Original_info_Wr++] =0x00;  //  补齐12   位
    //省域ID
    Original_info[Original_info_Wr++]= 1 ; //(u8)(JT808Conf_struct.Vechicle_Info.Dev_ProvinceID>>8);
    Original_info[Original_info_Wr++]= 0x10 ; //(u8)JT808Conf_struct.Vechicle_Info.Dev_ProvinceID;
   //县市域ID
    Original_info[Original_info_Wr++]=(u8)(1010>>8);
    Original_info[Original_info_Wr++]=(u8)1010;
    //制造商ID
    memcpy(Original_info+Original_info_Wr,"70420",5);                           
    Original_info_Wr+=5;
    //终端型号20个字节位数不足的后补0
    memcpy(Original_info+Original_info_Wr,"TW703",5);  
    Original_info_Wr+=5;      
    for(i=0;i<15;i++)
    {
    ...
    Original_info_Wr+=7; 
    //车牌颜色
    Original_info[Original_info_Wr++]=2;//  2  黄色JT808Conf_struct.Vechicle_Info.Dev_Color;
    //车牌号码
	//车牌
	memcpy(Original_info+Original_info_Wr,JT808Conf_struct.Vechicle_Info.Vech_Num,9);  
	Original_info_Wr+=9;
    //车辆vin
    memcpy(Original_info+Original_info_Wr,JT808Conf_struct.Vechicle_Info.Vech_VIN,17);
	Original_info_Wr+=17;
	// 获取加密信息长度 
	len_jiami =Original_info_Wr-start_jiami;//strlen(Original_info)-start_jiami+1;
	rt_kprintf("wo yao kai hu le ......"); 
	if(0x00 != data)//是否加密(不执行这个函数就正常)
		{ 
		    OutPrint_HEX("加密前信息",Original_info+start_jiami,len_jiami);
		                        // key      M1            IA1          IC1
			encrypt_lock2_encode(70420,22185791,23898412,87946901,Original_info+start_jiami,len_jiami);
			OutPrint_HEX("加密后信息",Original_info+start_jiami,len_jiami);
		}
	rt_kprintf("yi jing kai hu le ......"); 
	//  3. Send 
  ...
  if(Warn_Status[1]&0x10) 
   {
	...
	//  附加信息 7  -----------------------------	  
	 ...
	 Original_info[Original_info_Wr++]=Satelite_num;
	    ...
	    //  附加信号 10  ---------------------------------
	 Original_info[Original_info_Wr++]= 0x02; // 油量信息
	 Original_info[Original_info_Wr++]=2;  // len
	 Original_info[Original_info_Wr++]=(Oil_volum>>8);  
	 Original_info[Original_info_Wr++]=(Oil_volum);  

	 
     //  附加信号 11  ----------------------------------
     if(SleepState==1)
      {
          Original_info[Original_info_Wr++]= 0x2A; // I/O 状态
	      Original_info[Original_info_Wr++]=2;  // len
	      Original_info[Original_info_Wr++]=(Sleep_IO_state>>8);  
	      Original_info[Original_info_Wr++]=(Sleep_IO_state);   
      }
...
u8 Paramater_0106_stuff(u32 cmdid, u8 *deststr)
{
   ...
   switch (cmdid)
	...
	case 0x0043:
			...
			memcpy(( char * ) Original_info+ Original_info_Wr,"10086",5);
			Original_info_Wr+=5;
	        break;
    case 0x0044:	
   ...
   // 1. Head
	if(!Protocol_Head(MSG_0x0107,Packet_Normal)) 
	 ...
	 // 2. content   
       ...
       Original_info_Wr+=20;
	    memcpy( ( char * ) Original_info+ Original_info_Wr,(u8*)IMSI_CODE+8,7);
       Original_info_Wr+=7;	
   ...
   u8       QueryRecNum=0;  // 查询记录数目
	...
	switch( Recode_Obj.CMD )
	{
		...
		case 0x05:                                                                  //05      车辆信息采集
			...
			Original_info_Wr += 17;
			memcpy( Original_info + Original_info_Wr,JT808Conf_struct.Vechicle_Info.Vech_Num, 7 );               // 车牌号
			Original_info_Wr					+= 7;
			Original_info[Original_info_Wr++]	= 0x00;
			Original_info[Original_info_Wr++]	= 0x00;
			Original_info[Original_info_Wr++]	= 0x00;
		...
		/*	if(Recode_Obj.Current_pkt_num%2)
			{    
			 get_09h(Buf_700H_buffer);
			  memcpy(Original_info + Original_info_Wr,Buf_700H_buffer,333);
			}
			else
               memcpy(Original_info + Original_info_Wr,Buf_700H_buffer+333,333); 
			
			Original_info_Wr += 333;
            */
			 memset(Buf_700H_buffer,0,sizeof(Buf_700H_buffer));   
			 get_09h(Buf_700H_buffer);
			 memcpy(Original_info + Original_info_Wr,Buf_700H_buffer,666);
...
{  
    u32  listlen=215;
	 ...
	 // 2. content     
	  ...
	  memcpy(Original_info+Original_info_Wr,"电子运单",8);  
         Original_info_Wr+=8;		
...
{  
  ...
  //---------------------------------------------
   ...
   if(!Protocol_Head(MSG_0x0900,Packet_Normal)) 
	...
	// 3. 内容
	 // 串口1 信息
	   memcpy(Original_info+Original_info_Wr,RawTrans_Com1.RawData,RawTrans_Com1.datalen);
	   Original_info_Wr+=RawTrans_Com1.datalen; 
	//	4. Send 
    Protocol_End(Packet_Normal,0 ); 
   if(DispContent)
   	{
...
{  
  ...
  //---------------------------------------------
   ...
   if(!Protocol_Head(MSG_0x0900,Packet_Normal)) 
	...
	// 3. 内容
	 // 串口2  信息
	   memcpy(Original_info+Original_info_Wr,RawTrans_Com2.RawData,RawTrans_Com2.datalen);
	   Original_info_Wr+=RawTrans_Com2.datalen;    
	//	4. Send 
    Protocol_End(Packet_Normal,0 ); 
   if(DispContent)
   	{
					   	...
					   	{ 
					   	   case 0x0200:    //  对应位置消息的应答
								...
								//	{  	  rt_kprintf( "\r\n  0200-2"); break;}
									...
									//----- app msg ----------
								      if((AppQue.sd_enable_flag==1)&&(app_que_enable==1))
								      	{
								      	    AppQue.sd_enable_flag=0;
									    AppQue.abnormal_counter=0; 	

									    if(AppQue.re_send_flag)	
                                                                    {
                                                                         AppQue.re_send_flag=0; // clear  
                                                                         rt_kprintf("\r\n resend->not add");   
									     }									 
								           else		
									           AppQue.read_num++;																	 
						                ...
						                if(0==UDP_HEX_Rx[17])
                                         {  // 鉴权成功 
					                        ...
					                        if(DataLink_Status())   
											 ...
											 SD_ACKflag.f_Worklist_SD_0701H=1;  //  电子运单
						                }
									   break;
...
void SpeedWarnJudge(void)  //  速度报警判断 
{
              ...
              if(SleepState==1)//休眠
              {
                     if(GPS_speed>100 )	// 速度大于10 km
                     {
                         spd_sleepDur++;
						 if(spd_sleepDur>=10) //  速度大于10s
						 {
							   if(spd_sleepDur==10)
							   {
									if((Warn_Status[3]&0x10)==0x00)
									{
			                             //-------- 非法位移了----
										 Warn_Status[3]|=0x10;
										 PositionSD_Enable();  
					 ...
					 if(  JT808Conf_struct.Speed_warn_MAX >0 )   //> 0 
					 {
						 ...
						 //----- GPS  即时速度	0.1 km/h  ---------
							...
							// if( DebugSpd > ( JT808Conf_struct.Speed_warn_MAX*10) ) 
							 ...
							 if(GPS_speed<(JT808Conf_struct.Speed_warn_MAX*10)&&(GPS_speed>(JT808Conf_struct.Speed_warn_MAX*10-JT808Conf_struct.BD_MaxSpd_preWarnValue)))
							 {
							       if((Warn_Status[2]&0x20)==0x00)
							       	{
							       	       rt_kprintf("\r\n  超速预报警触发\r\n"); 
										         StatusReg_SPD_PreWARN();
                                           PositionSD_Enable();  
				                           Current_UDP_sd=1;  
							       	}
...
void  Sleep_Mode_ConfigEnter(void)
{
   if(SleepState==0)
		{
		   	 SleepCounter++;
			 if(SleepCounter>18000)   //15 old 防抖处理      30min  1800s     18000 
			 {
			      SleepCounter=0; 
			   	   SleepState=1;		
				   Sleep_IO_state=0x0002;  // 休眠状态 表32  bit 1  
				   if(JT808Conf_struct.RT_LOCK.Lock_state!=1)
				  ...
				  */  
				   if(DataLink_Status())
					...
					SD_ACKflag.f_Worklist_SD_0701H=1; // 上报电子运单
			 }  
		 }
}
...
void Tired_Check(void)
{
	         ...
	         // if(DebugSpd>60) 
			  {   								 
				 ...
				 //---------------------------------  
				 if( TiredConf_struct.Tired_drive.ACC_ONstate_counter==(TiredConf_struct.TiredDoor.Door_DrvKeepingSec-30)) //提前5分钟蜂鸣器提示注意疲劳驾驶 14100
					{  
					     TiredConf_struct.Tired_drive.Tgvoice_play=1;
					     TTS_play("您还有20秒就疲劳驾驶，请注意安全");	 
				 	}
                 //----------- 疲劳驾驶预报警  ----------------------------
                 if( TiredConf_struct.Tired_drive.ACC_ONstate_counter>=(TiredConf_struct.TiredDoor.Door_DrvKeepingSec-JT808Conf_struct.BD_TiredDrv_preWarnValue))
                 	{
                 	    if(TiredConf_struct.Tired_drive.ACC_ONstate_counter<TiredConf_struct.TiredDoor.Door_DrvKeepingSec)
					   {
						    if(((Warn_Status[2]&0x40)==0x00)&&((Warn_Status[3]&0x04)==0x00))
					     	{
								  rt_kprintf("\r\n  疲劳预报警触发\r\n"); 
								  StatusReg_Tired_PreWARN();
								   PositionSD_Enable();  
					               Current_UDP_sd=1;  
						    }
                 	   }
				 ...
				 TiredConf_struct.Tired_drive.Flag=1;   
				  ...
				  //-------- 当天累计驾驶时间判断 -------------------------
				  if(JT808Conf_struct.PositionSd_Stratage==0)
				  {
				      //----------------------------------------------------------------------
                      TiredConf_struct.Tired_drive.CurrentDay_DriveTimer_0++;
					  if(TiredConf_struct.Tired_drive.CurrentDay_DriveTimer_0>= TiredConf_struct.TiredDoor.Door_DayAccumlateDrvSec)
					  	{					  	    
							if(TiredConf_struct.Tired_drive.CurrentDay_DriveTimer_0== TiredConf_struct.TiredDoor.Door_DayAccumlateDrvSec)
							{
                              	Warn_Status[1]|=0x04;  //BIT(3)  当天累计驾驶超时
								//---- 触发即时上报数据-------
								PositionSD_Enable();    
						        Current_UDP_sd=1;	
							}
				  ...
				  if(JT808Conf_struct.PositionSd_Stratage==1)
				  {
				      //     Driver 1 
				      if(strncmp(JT808Conf_struct.Driver_Info.BD_DriveName,"张三三",6)==0)
				      {
						  //----------------------------------------------------------------------
						  TiredConf_struct.Tired_drive.CurrentDay_DriveTimer_Driver_1++;
						  if(TiredConf_struct.Tired_drive.CurrentDay_DriveTimer_Driver_1>= TiredConf_struct.TiredDoor.Door_DayAccumlateDrvSec)
							{							
								if(TiredConf_struct.Tired_drive.CurrentDay_DriveTimer_0== TiredConf_struct.TiredDoor.Door_DayAccumlateDrvSec)
								{
									Warn_Status[1]|=0x04;  //BIT(3)  当天累计驾驶超时
                      ...
                      //     Driver 2 
				      if(strncmp(JT808Conf_struct.Driver_Info.BD_DriveName,"李四四",6)==0)
				      	{
						  //----------------------------------------------------------------------
						  TiredConf_struct.Tired_drive.CurrentDay_DriveTimer_Driver_2++; 
						  if(TiredConf_struct.Tired_drive.CurrentDay_DriveTimer_Driver_2>= TiredConf_struct.TiredDoor.Door_DayAccumlateDrvSec)
							{							
								if(TiredConf_struct.Tired_drive.CurrentDay_DriveTimer_0== TiredConf_struct.TiredDoor.Door_DayAccumlateDrvSec)
								{
									Warn_Status[1]|=0x04;  //BIT(3)  当天累计驾驶超时 
			  ...
			  { 
			    ...
			    if(TiredConf_struct.Tired_drive.Flag==1)  //只有从ACCon切换过来时才做以下处理，如果长期关着，没有必要了 
				 ...
				 //----------------------------------------------------------------				   
				   if(((Warn_Status[2]&0x40)==0x00)&&(TiredConf_struct.Tired_drive.ACC_ONstate_counter >= TiredConf_struct.TiredDoor.Door_DrvKeepingSec)) //14400 // 连续驾驶超过4小时算疲劳驾驶
				   {   // 不在预警情况下 
...
void  dur(u8 *content)
{
  ...
  rt_kprintf("\r\n 手动设置上报时间间隔 %d s\r\n",Current_SD_Duration);
	   ...
	   GNSS_rawdata.SendDuration=Current_SD_Duration;
        Api_Config_Recwrite_Large(jt808,0,(u8*)&JT808Conf_struct,sizeof(JT808Conf_struct));
...
void rec_mov(void)
{
   ...
   WatchDog_Feed();
       rt_kprintf("\r\n erase");
      SST25V_BlockErase_64KByte((CycleStart_offset)*PageSIZE);      // erase Sector		
      rt_kprintf("  Over\r\n");
	for(i=0;i<256*PageSIZE;i++)
     {
     
	    DF_ReadFlash(CycleStart_offset,i,(u8*)&C,1); 	
	    WatchDog_Feed();
	    DF_WriteFlash(DF_Record_Page,i,(u8*)&C,1); 	          

     }
}
FINSH_FUNCTION_EXPORT(rec_mov, rec_mov);  
...
void rec_rd(void)
{
     u32  i=0; 
      ...
      for(i=0;i<256*PageSIZE;i++)
      {     
	    DF_ReadFlash(DF_Record_Page,i,(u8*)&C,1);
	    rt_kprintf(" %2X",C);	
	    WatchDog_Feed();	
      }  
      rt_kprintf("\r\n");	 	
}
FINSH_FUNCTION_EXPORT(rec_rd, rec_rd);  
...
FINSH_FUNCTION_EXPORT(open_com, open_com);   
...
FINSH_FUNCTION_EXPORT(sensorspd, sensorspd1);    

void qianya(u8 *instr)
{

	if(instr[0]=='0')
	   {
			 Warn_Status[3]&=~0x80;  // bit 7
...
FINSH_FUNCTION_EXPORT(qianya, qianya);    

void lcd(u8 *instr)
{
	if(instr[0]=='0')
	   {
			 Warn_Status[2]&=~0x02;  // bit 9
...
FINSH_FUNCTION_EXPORT(lcd, lcd);    

void tts(u8 *instr)
{
	if(instr[0]=='0')
	   {
			 Warn_Status[2]&=~0x04;  // bit 10
...
FINSH_FUNCTION_EXPORT(tts, tts);    

void ic(u8 *instr)
{
	if(instr[0]=='0')
	   {
			 Warn_Status[2]&=~0x10;  // bit 12
...
FINSH_FUNCTION_EXPORT(ic, ic);    

void vss(u8 *instr)
{
     if(instr[0]=='0')
     	{
		      Warn_Status[0]&=~0x01;  // bit 24
...
FINSH_FUNCTION_EXPORT(vss, vss);    

void tired_clear(u8 *instr)
{   
    memset(JT808Conf_struct.Driver_Info.BD_DriveName,0,sizeof(JT808Conf_struct.Driver_Info.BD_DriveName)); 
	memcpy(JT808Conf_struct.Driver_Info.BD_DriveName,instr,strlen(instr)); 
    Api_Config_Recwrite_Large(jt808,0,(u8*)&JT808Conf_struct,sizeof(JT808Conf_struct));		

	Warn_Status[1]&=~0x04;  //BIT(18)  当天累计驾驶超时  清除 
	//--------------------------------
    TIRED_Drive_Init();

}
FINSH_FUNCTION_EXPORT(tired_clear, tired_clear);   
...
void oilvalue(u8 *instr)
{

   sscanf(instr, "%d", (u32*)&Oil_volum);
   rt_kprintf("\r\n set current Oil volum= %d 1/10L\r\n",Oil_volum);   

}
FINSH_FUNCTION_EXPORT(oilvalue, oilvalue set );   

void com1_data(u8 *instr)
{
  u16 i=0;

   RawTrans_Com1.datalen=strlen(instr);
   memcpy(RawTrans_Com1.RawData,instr,RawTrans_Com1.datalen);
   RawTrans_Com1.Enable_SD_Flag=1;

    rt_kprintf("\r\n com1 rx:");
	for(i=0;i<RawTrans_Com1.datalen;i++)
		{
               rt_kprintf("%c",RawTrans_Com1.RawData[i]);
		}
	 rt_kprintf("\r\n");

}
FINSH_FUNCTION_EXPORT(com1_data, com1_data );   

void com2_data(u8 *instr)
{
  u16 i=0;
  
   RawTrans_Com2.datalen=strlen(instr);
   memcpy(RawTrans_Com2.RawData,instr,RawTrans_Com2.datalen);
   RawTrans_Com2.Enable_SD_Flag=1;

     rt_kprintf("\r\n com2 rx:");  
	for(i=0;i<RawTrans_Com2.datalen;i++)  
		{
               rt_kprintf("%c",RawTrans_Com2.RawData[i]); 
		}
	 rt_kprintf("\r\n");

}
FINSH_FUNCTION_EXPORT(com2_data, com2_data );    
...
FINSH_FUNCTION_EXPORT(handsms, handsms);  
 ...
 void  warn_msg_sd(void)
 {
     u8  SMS_REGstr[70];

	 memset(SMS_REGstr,0,70);
	 strcpy(SMS_REGstr,JT808Conf_struct.Vechicle_Info.Vech_Num);
     strcat(SMS_REGstr,"|");
	 memcpy(SMS_REGstr+strlen(SMS_REGstr),JT808Conf_struct.Vechicle_Info.Vech_sim,12);
	 strcat(SMS_REGstr,"|EmgWarn|");
	 sprintf(SMS_REGstr+strlen(SMS_REGstr),"%s|%s|%s",Posit_Str.DateTime,Posit_Str.Lati_Str,Posit_Str.Longi_Str); 
 ...
 FINSH_FUNCTION_EXPORT(warn_msg_sd, warn_msg_sd);   
 ...
 FINSH_FUNCTION_EXPORT(encode, encode);  
