...
ALIGN(RT_ALIGN_SIZE)
 uint8_t					APP_serialinfo[APP_RAWINFO_SIZE];
 struct rt_messagequeue	mq_APPs; 
...
void   DialLink_TimeOut_Process(void)
{
           if( DataDial.start_dial_stateFLAG==1) 
	   {
					  ...
					  if(DataDial.start_dial_counter>1800)  //300  	 
					  {
...
void _gsm_startup(void)
{
	  ...
	  rt_mq_init( &mq_APPs, "mq_APPs", &APP_serialinfo[0], 30 - sizeof( void* ), APP_RAWINFO_SIZE, RT_IPC_FLAG_FIFO );     
