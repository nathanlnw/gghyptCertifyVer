...
static char DialStr_IC_card[50]="AT%IPOPENX=2,\"TCP\",\""; //  远程升级            
...
//  M66  Rec voice 
flash char VoiceRec_config[]="AT%RECCFG=\"AMR\",0,20\r\n";  // 设置录音格式和录音质量  
                                                                                                 //  0C: 28 长度 18组 504
      ...
      switch(VocREC.Sate)
      {
           case VOICEREC_RECORDING:   
							           ...
							           {  
										...
										memcpy(VocREC.SendString+str_wr,"\",15\r\n",6);//  5s  voice  data        
                                                                      str_wr+=5;
										// debug
										rt_kprintf("\r\n %s",VocREC.SendString); 
		 ...
		 //  1. 获取长度	  
		  ...
		  //          rt_kprintf("%c",instr[i+1+j]);   
		   ...
		   if(VocREC.HEX_len!=(len>>1))
		    ...
		    // 3.  write and  check
	                 DF_WriteFlashDirect( VocREC.curren_PageCNT,VocREC.file_write,VocREC.HEX, VocREC.HEX_len); 
                         //---  read compare 
			    memset(pic_buf,0,sizeof(pic_buf)); 
			    DF_ReadFlash(VocREC.curren_PageCNT,VocREC.file_write,pic_buf, VocREC.HEX_len);      
...
{    
    	    if((CSQ_flag==1)&&(MediaObj.Media_transmittingFlag==0)&&(GSM_PWR.GSM_power_over)&&(stopNormal==0))  
	   { 
		  CSQ_flag=0; 
...
//----------------------  Debug -------------------------		
  ...
  if(BD_ISP.ISP_running==0)
   ...
   /*      if((SMS_send.Msg_sdState==1)&&(SMS_send.Msg_step==2))
      ...
      //--------------------------------------------------------
	 if(strncmp((char*)GSM_rx,"%RECDATA:",9)==0)  //%RECDATA:18,"0C49CA45E3F24DF500BE1   x28
      	{                                                                            //%RECDATA:2,"0C341
      	   //   rt_kprintf("\r\n  RECFGET len=%d\r\n",len);
      	      WatchDog_Feed();  
         ...
         if(strncmp((char*)GSM_rx,"%RECSTOP:",9)==0)    //%RECSTOP: 10,7000 
         {
             WatchDog_Feed();
             rt_kprintf("\r\n 接收 录音结束\r\n");         	
	      if(VocREC.Sate==VOICEREC_DataRXing) 	    // 如果处于数据接收态则处理否则不处理
                   VOC_REC_Stop(GSM_rx);    
		WatchDog_Feed();	 
	      goto RXOVER; 
         }
	      	...
	      	case Dial_AuxLnk	       :	if (failed || error)
								...
								{   //  below  run once  since  online
								     ...
								     if(DataDial.Dial_step ==Dial_AuxLnk)    //auxlink 	 
								            ...
								            //   注册状态  且 不是休眠状态
								            if((1==JT808Conf_struct.Regsiter_Status)&&(SleepState==0))  
								                {  
