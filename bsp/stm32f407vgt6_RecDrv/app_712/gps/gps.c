		...
		if(*packet==','){
			...
			switch(CommaCount){
				...
				case 4://纬度
					...
					memset(Posit_Str.Lati_Str,0,sizeof(Posit_Str.Lati_Str)); 
					memcpy(Posit_Str.Lati_Str,tmpinfo,9); 
		...
		if((*packet==',')||(*packet=='*')){  
			...
			switch(CommaCount){
				case 5:
						...
						if(strncmp((char*)tmpinfo,"ANTENNA OK",10)==0)	//ANTENNA SHORT
						{
...
void  gnss_timer(void)
{
      if(!GNSS_rawdata.WorkEnable)
            return;

	    
		GNSS_rawdata.SendCounter++;
        if(GNSS_rawdata.SendCounter>=GNSS_rawdata.SendDuration)
        	{        	  
			  GNSS_rawdata.SendCounter=0;
			  GNSS_rawdata.SendFlag=1;      
        	}
